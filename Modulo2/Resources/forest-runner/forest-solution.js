...
function makeGroundMap(base, variance) {
  ...
  for (let i = 0; i < image.data.length; i += 4) {
    const value = Math.max(0, Math.min(255, base + (Math.random() - 0.5) * variance))
    image.data[i] = value
...
const roughnessMap = makeGroundMap(210, 70)
...
const metalnessMap = makeGroundMap(4, 4)
